...
def synth_generate():
        ...
        df = pd.read_csv("pybo/uploads/" + obj + ".csv", header=None, skiprows = 1, names=columns).astype(dtypes)
        # 헤더가 있는 경우 -> skip
        ...
        synth_df.to_csv("pybo/synth_dir/" + obj + ".csv", index= False)
        return render_template('synth_generate.html', df=[synth_df2.to_html()], dtypes=synth_df.dtypes,titles = [''])
...
def partsynth_generate():
        ...
        result.to_csv("pybo/synth_dir/" + obj + ".csv", index= False)
        return render_template('partsynth_generate.html', df=[result2.to_html()], dtypes=result2.dtypes,titles = [''])
